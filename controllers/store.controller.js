...
exports.resize = async (req, res, next) => {
  ...
  // Once we have written the photo to the file system. Continue
  next();
};
...
exports.getStores = async (req, res) => {
  ...
  res.render('stores', { title: 'Store 1', stores, page, pages, count });
};
...
exports.editStore = async (req, res) => {
  ...
  //3. Render out the edit form so the user can update their stores
  res.render('editStore', { title: `Edit ${store.name}`, store });
};
function confirmOwner(store, user) {
...
exports.updateStore = async (req, res) => {
  ...
  //2. Tell them it worked
  req.flash(
    'success',
    `Successfully updated <strong>${store.name}</strong>. <a href="/stores/${store.slug}">View store</a>`
  );

  //3. Redirect to the store
...
exports.mapPage = (req, res) => {
  res.render('map', { title: 'Stores map' });
};

exports.heartStore = async (req, res) => {
  //1. Check if  the user has already marked this store
